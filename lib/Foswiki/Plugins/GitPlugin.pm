...
use vars qw( $VERSION $RELEASE $SHORTDESCRIPTION $debug $pluginName $NO_PREFS_IN_TOPIC $SESSION);
...
sub _GITMERGE {
	...
	( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{fetchCmd}, SITENAME => $Foswiki::cfg{Git}{backupReposName} );
    ...
    $totalInfo .= "<strong>warning:</strong> Can not read $GitDataPath/data, when trying to rmove .changes files.<br>" unless ( opendir( my $GitDIR, $GitDataPath) );
	...
	if ($Foswiki::cfg{Git}{mergeMode} eq "distributed") {
	    ( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{fetchCmd}, SITENAME => $location );
	    $gitOut = "No more data.\n" if ( $gitOut eq '' );
	    $gitOut = "<strong>get fetch result:</strong>\n".$gitOut;
	    $gitOut =~ s/\n/<br>/go;
	    $totalInfo .= $gitOut."<br>";
	    ( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{mergeCmdDistributed}, SITENAME => $location );
	} elsif ($Foswiki::cfg{Git}{mergeMode} eq "centralized") {
	    $location = $Foswiki::cfg{Git}{backupReposName}."/".$location;
	    ( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{mergeCmdCentralized}, SITENAME => $location );
	}
	my $mergeInfo = $gitOut;

	#git ls-files --unmerged, if no files are unmerged, success.
	( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{unmergeListCmd}, SITENAME => $location );
	...
	for( my $i=0; $i<$#conflictInfo+1; $i+=4) {
	    push( @sortedInfo, [$conflictInfo[$i], $conflictInfo[$i+1], $conflictInfo[$i+2],$conflictInfo[$i+3] ] );
	}
	...
	foreach my $curFile (sort keys %conflictTxt) {
		...
		( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{showAncestorConfictCmd}, FILENAME => $curFile );
		$ancestorFile = $gitOut;
	    }
	    
	    if( defined($conflictTxt{$curFile}->{2}) ) {
		( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{showHEADConfictCmd}, FILENAME => $curFile );
		$headFile = $gitOut;
	    }
	    
	    if( defined($conflictTxt{$curFile}->{3}) ) {
		( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{showMERGE_HEADConfictCmd}, FILENAME => $curFile );
		...
		my( $bMeta, $bText) = _readTopicFromTopicRaw($session, $web, $topic, $headFile );
		my( $cMeta, $cText) = _readTopicFromTopicRaw($session, $web, $topic, $mergeheadFile );

		if( defined $ancestorFile ) {
		    ( $aMeta, $aText) = _readTopicFromTopicRaw($session, $web, $topic, $ancestorFile );
		}
		...
		my $resolvedText = _merge("Ancestor", $aText, "Local", $bText, "$location", $cText, ".*?\\n", $session);
		#my $resolvedText = Foswiki::Merge::merge3("Ancestor", $aText, "Local", $bText, "$location", $cText, ".*?\\n", $session);

		my ( $resolvedMeta, @attachment_Info ) = _mergeMeta($bMeta, $cMeta, $web, $topic, \%conflictBin, $location);
		...
		Foswiki::Func::saveTopic( $web, $topic, $resolvedMeta, $resolvedText );
		
		$web =~ s#/#\.#go;
		$totalInfo .= "Please check merged file: $web.$topic in !$web<BR>";
	    } elsif ( ( defined $headFile and (not defined $mergeheadFile ) and (not defined $ancestorFile ) ) 
		   or ( defined $mergeheadFile and (not defined $headFile ) and (not defined $ancestorFile ) ) ) {
		#the file was moved to here
		my( $Meta, $Text);
		if( defined $headFile ) {
		    ($Meta, $Text) = _readTopicFromTopicRaw($session, $web, $topic, $headFile );
		} elsif ( defined $mergeheadFile ) {
		    ($Meta, $Text) = _readTopicFromTopicRaw($session, $web, $topic, $mergeheadFile );
		}

		if( scalar $Meta->find('FILEATTACHMENT') > 0 ) {
		    my @tmpArray = $Meta->find('FILEATTACHMENT');
		    foreach my $attName ( @tmpArray ) {
			my $attachPath = "pub/"."$web"."/"."$topic"."/".$attName->{name};
			( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{initCmd}, FILENAME => $attachPath );
			delete $conflictBin{"$attachPath"};
		    }
		}
		( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{initCmd}, FILENAME => $curFile );
	    } elsif ( defined $ancestorFile and (not defined $mergeheadFile ) and (not defined $headFile ) ) {
		#the file was removed/deleted in both sides
		my( $Meta, $Text) = _readTopicFromTopicRaw($session, $web, $topic, $ancestorFile );
		if( scalar $Meta->find('FILEATTACHMENT') > 0 ) {
		    my @tmpArray = $Meta->find('FILEATTACHMENT');
		    foreach my $attName ( @tmpArray ) {
			my $attachPath = "pub/"."$web"."/"."$topic"."/".$attName->{name};
			( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{removeCmd}, FILENAME => $attachPath );
			delete $conflictBin{"$attachPath"};
		    }
		}

		( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{removeCmd}, FILENAME => $curFile );
	    } else {
	...
	( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{mergeConfictCmd} );
    ...
    #"git status"
    $totalInfo .= "<strong>Please check information below, any problem you met, solve them using command line.</strong><BR><BR>";

    ( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{statusCmd} );
    $gitOut =~ s/\n/<br>/go;
    $totalInfo .= $gitOut;

    if( $Foswiki::cfg{Git}{mergeMode} eq "centralized" ) {
	( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{pushCmd}, REPOSNAME=>$Foswiki::cfg{Git}{backupReposName}, LOCALNAME=>$Foswiki::cfg{Git}{LocalName} );
    }
...
sub _mergeMeta {
    ...
    if( defined $bMeta->get('TOPICMOVED') and defined $cMeta->get('TOPICMOVED') ) {
	#latest win
	if( $bMeta->get('TOPICMOVED')->{date} < $cMeta->get('TOPICMOVED')->{date} ) {
    ...
    my $flagMatched;
    if ( scalar $bMeta->find('FILEATTACHMENT') > 0 or scalar $cMeta->find('FILEATTACHMENT') > 0 ) {
	...
	foreach my $cAttachment ( @tmpArrayC )	{
		...
		if( $bAttachment->{name} eq $cAttachment->{name} ) {
			...
			$pickupVersion = ((0x1 xor ($baseVersion-2) ) + 2);		                #convert 2<-->3
		    } else {
		    ...
		    _mergeAttachment($web, $topic, $bAttachment->{name}, $pickupVersion, $refBin );
	...
	foreach my $bAttachment( @tmpArrayB ) {
	    ...
	    my $attachPath = $Foswiki::cfg{PubDir}."/$web/$topic/$attachmentName";
	    unless( -e $attachPath ) {
		for (my $i=0; $i<$#attachmentTmp+1; $i+=3) {
    ...
    my $topicPath = $Foswiki::cfg{Git}{root}.'/'."data"."/$web/$topic".".txt";
...
sub _mergeAttachment {
    ...
    if( defined $refBin->{"$attachPath"} and (my $sh1 = $refBin->{"$attachPath"}{$pickupVersion}) ) {
	my ( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render($Foswiki::cfg{Git}{readFileCmd}, SHA1 => $sh1);

	my $tmpPath = $Foswiki::cfg{PubDir}.'/'.$web.'/'.$topic.'/'.$attachmentName;
...
sub _merge {
    ...
    for my $diff (@diffs) {
		...
		my $merged = dispatch( $session, 'mergeHandler',
				       ' ',  $dline, $dline, $info );
                if( defined $merged ) {
                    push( @out, $merged );
                } else {
...
sub _changeForm {
    ...
    my ( $gitOut, $exit ) = Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{logCmd}, TIME => $Foswiki::cfg{Store}{RememberChangesFor} );
    my %webChange;
	...
	foreach my $webName ( keys %webChange )	{
		...
		return "<strong>warning:</strong> can not wirte $path file.<br>";
	    }
	}
	return;
    }
    return "<strong>warning:</strong> can't get repository's log infomation. It failed when try to run 'git log'.";
}
...
sub _getChangeInfo {
    ...
    if ( $commit =~ m/\s(.*?)\s(.*?)TW_Version:([0-9]*)\s(.*?)TW_Author:([0-9a-zA-Z_]*)\s(.*?)TW_Path:(.*?)\s(.*?)TW_Comment:(.*?)\s(.*?)TW_Date:(.*?)\s/s ) {
...
sub _replaceLink {
    ...
    return "<strong>warning:</strong> Can not read $GitPath/$little.<br>" unless ( opendir( $GitDIR, $GitPath.'/'.$little ) );
    return "<strong>warning:</strong> Can not read $Foswiki::cfg{$directory}.<br>" unless ( opendir( $FoswikiDIR, $Foswiki::cfg{$directory} ) );
	...
	next unless ( $link =~ m#$GitPath/$little# );	#skip the soft link that does not point to <Gitroot> /date or /pub.
...
sub _rmChanges {
	...
	my $filePath = $path.'/'.$1;
	if( -d $filePath ) {
	    return "<strong>warning:</strong> Can not read $filePath.<br>" unless ( opendir( my $FileDIR, $filePath) );
