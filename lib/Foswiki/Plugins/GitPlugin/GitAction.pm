...
sub handleGitConfirm {
    ...
    my $confirmButton =  " <form action=\"" . &Foswiki::Func::getScriptUrl($session->{webName}, $session->{topicName}, 'gitop') . "\" /><input type=\"hidden\" name=\"nr\" value=\"$cnt\" /><input type=\"hidden\" name=\"opertype\" value=\"confirm\" /><input type=\"submit\" value=\"Confirm\" /></form>";

    my $warning_start = qq#<div class="foswikiBroadcastMessage">$confirmButton<img src="/pub/System/DocumentGraphics/warning.gif" alt="ALERT!" title="ALERT!" border="0" height="16" width="16"> <strong> #;

    my $warning_content = qq#Merge Conflict. Please CONFIRM the content and attachment.#;
...
sub handleGitOption {
    ...
    my $OptionButton =  " <form action=\"" . &Foswiki::Func::getScriptUrl($session->{webName}, $session->{topicName}, 'gitop') . "\" /><input type=\"hidden\" name=\"lo\" value=\"$attr\" /><input type=\"hidden\" name=\"opertype\" value=\"option\" /><input type=\"hidden\" name=\"nr\" value=\"$cnt\" /><input type=\"submit\" value=\"Select\" /></form>";
	...
	my $warning = qq#<div>$OptionButton<strong>CONFLICT version $attr:</strong></div>#;
	return $warning;
    }    
}     
...
sub oper {
	...
	$text =~ s/%GITCONFIRM(?:{(.*?)})?%/&removeConfirm($cnt--, $user, $1)/geo;
    ...
    my $error = &Foswiki::Func::saveTopic( $webName, $topic, $meta, $text, { forcenewrevision => 1 } );	
	...
	Foswiki::Func::redirectCgiQuery( $query, &Foswiki::Func::getViewUrl( $webName, $topic ) );
    }
...
sub doEnableEdit {
    ...
    if( ! &Foswiki::Func::checkAccessPermission( "change", $user, "", $theTopic, $theWeb ) ) {
    ...
    my( $oopsUrl, $lockUser ) = &Foswiki::Func::checkTopicEditLock( $theWeb, $theTopic, 'edit' );
    if( $lockUser && ! ( $lockUser eq Foswiki::Func::getCanonicalUserID($user) ) ) {
