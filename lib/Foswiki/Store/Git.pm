    ...
    if ($topic) {
        ...
        if ($attachment) {
            ...
            $this->{file} =
              ...
              . $attachment;
            $this->{relfile} =
              "pub" . '/' . $web . '/' . $this->{topic} . '/' . $attachment;
            $this->{gitfile} =
              $Foswiki::cfg{Git}{root} . '/' . $this->{relfile};
...
sub getRevision {
    ...
    my $sha1;
    my $path
      ; #because after file rename, we lost the old name, we stored it in comment
...
sub _saveFile {
    ...
    my ( $gitOutput, $exit ) =
      Foswiki::Plugins::GitPlugin::GitRun::render( $Foswiki::cfg{Git}{initCmd},
        FILENAME => $this->{relfile} );
    if ($exit) {
        throw Error::Simple( $Foswiki::cfg{Git}{initCmd} . ' of '
              . $this->_hidePath( $this->{file} )
              . ' failed: '
              . $gitOutput );
    }
...
sub moveTopic {
    ...
    # Move data file
    my $new =
      new Foswiki::Store::Git( $this->{session}, $newWeb, $newTopic, '' );

#judeg from web and to web, all will use Git, or all will not , or one will and one will not.
    if (
        _isLocalWeb( $this->{web} )
        and
        ( _isLocalWeb( $new->{web} ) or $newWeb eq $Foswiki::cfg{TrashWebName} )
      )
    {
        ...
        my $info =
"could not move topic $this->{topic} from !$this->{web} to !$newWeb, because one is out-sync web and the other is not.";
...
sub _localMoveFile {
    ...
    my $toWeb = $1
      if ( ( $to =~ m#^$Foswiki::cfg{PubDir}/(.*)# )
        or ( $to =~ m#^$Foswiki::cfg{DataDir}/(.*)# ) );
    $toWeb = $1 if ( $toWeb =~ m#^(.*?)/# );
...
#If the web, which will not use Git, it will be deleted at now. you can replace codes here.
    if ( $toWeb eq $Foswiki::cfg{TrashWebName} ) {
        my $localTrash =
          $Foswiki::cfg{DataDir} . '/' . $Foswiki::cfg{Git}{LocalTrashWebName};
        _localCreateTrash($this) unless ( -e $localTrash );
        $to =~
s#^($Foswiki::cfg{PubDir}/)$Foswiki::cfg{TrashWebName}#$1$Foswiki::cfg{Git}{LocalTrashWebName}#;
        $to =~
s#^($Foswiki::cfg{DataDir}/)$Foswiki::cfg{TrashWebName}#$1$Foswiki::cfg{Git}{LocalTrashWebName}#;
        _mkPathTo($to);
        unless ( File::Copy::move( $from, $to ) ) {
            throw Error::Simple(
                'localWeb: move ' . $from . ' to ' . $to . ' failed: ' . $! );
        }
    }
    else {
        _mkPathTo($to);
    ...
    if ( _isLocalWeb( $new->{web} ) ) {
          ...
          )
        {
            for my $att ( grep { !/^\./ } readdir $DIR ) {
                ...
                my $oldAtt =
                  new Foswiki::Store::Git( $this->{session}, $this->{web},
                    $this->{topic}, $att );
...
sub getStream {
    ...
    unless ( open( $strm, '<' . $this->{file} ) ) {
        throw Error::Simple(
            'HG: stream open ' . $this->{file} . ' failed: ' . $! );
    }
    return $strm;
}
...
sub _saveStream {
    ...
    binmode(F)
      || throw Error::Simple(
        'HG: failed to binmode ' . $this->{file} . ': ' . $! );
    my $text;
    binmode(F);
...
sub moveAttachment {
    ...
    my $new =
      Foswiki::Store::Git->new( $this->{session}, $newWeb, $newTopic,
        $newAttachment );
...
#Judeg from web and to web, all will use Git, or all will not , or one will and one will not.
      ...
      )
    {
        ...
        my $info =
"could not move attachment $this->{attachment} from !$this->{web} to !$newWeb, because one is out-sync web and the other is not.";
        throw Error::Simple( "permission deney:" . $info );
    }
}
...
#Judeg from web and to web, all will use Git, or all will not , or one will and one will not.
    if ( _isLocalWeb( $this->{web} ) and _isLocalWeb($newWeb) ) {
        ...
        _moveFile(
            $Foswiki::cfg{Git}{root} . '/' . 'data' . '/' . $this->{web},
        ...
        if ( -d $Foswiki::cfg{Git}{root} . '/' . 'pub' . '/' . $this->{web} ) {
            _moveFile(
                $Foswiki::cfg{Git}{root} . '/' . 'pub' . '/' . $this->{web},
        ...
        my $info =
"could not move !$this->{web} to !$newWeb, because one is out-sync web and the other is not.";
        throw Error::Simple( "permission deney:" . $info );
    }
}
...
sub _ci {
    ...
    my $gitComment =
"TW_Version:$this->{version}\tTW_Author:$this->{session}->{user}\tTW_Path:$this->{relfile}\tTW_Comment:$comment\tTW_Date:$date";
...
sub _mkPathTo {
    ...
    eval { File::Path::mkpath( $path, 0, $Foswiki::cfg{Git}{dirPermission} ); };
    chmod( 0775, $path );
    if ($@) {
...
sub _localCreateTrash {
    ...
    $new->createWeb(
        $this->{session}->{user},
        $Foswiki::cfg{Git}{LocalTrashWebName},
        $Foswiki::cfg{TrashWebName}, $opts
    );
